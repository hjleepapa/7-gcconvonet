...
**Hanok Table and this repo:** Convonet does **not** vendor or submodule the Hanok application. Point **agent-llm-service** at the running API with **`HANOK_MCP_API_BASE_URL`** (or **`HANOK_PUBLIC_BASE_URL`**) = `https://hanok.convonetai.com` (no trailing slash). The agent-llm container must still be able to **`import hanok_table`** for the stdio MCP shim (`db_hanok_table.py`); for local dev you may keep an optional **`kfood/`** directory (gitignored) on `PYTHONPATH`, or install a private wheel—see `convonet/mcps/local_servers/db_hanok_table.py`. Do not set **`HANOK_MCP_DISABLE`** unless you intentionally skip Hanok MCP.
