...
| **agent-llm-service** | `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_DB=0`, `DB_URI`; LLM: `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_MODEL`; optional for provider checks: `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY`, `CARTESIA_API_KEY`, `CARTESIA_*`. **Hanok:** `HANOK_MCP_API_BASE_URL` = your deployed Hanok API origin (e.g. `https://hanok.convonetai.com`). The image must **`import hanok_table`** for MCP stdio (extend `docker/agent-llm.Dockerfile` with a wheel/copy if needed). Unset URL drops Hanok MCP at runtime (`convonet/routes.py` `_filter_mcp_servers_for_runtime`). |
