...
**Hanok Table (separate Cloud Run service):** Build with `docker/hanok-table.Dockerfile` (see `cloudbuild.yaml` or `cloudbuild-hanok-table.yaml`). Before Cloud Build, run `git submodule update --init kfood` so `kfood/hanok_table` is present. Map **`hanok.convonetai.com`** (or your hostname) in Cloudflare to the Cloud Run URL. On **agent-llm-service**, set **`HANOK_MCP_API_BASE_URL`** (or **`HANOK_PUBLIC_BASE_URL`**) to that same HTTPS origin (no trailing slash), e.g. `https://hanok.convonetai.com`, so Convonet’s MCP stdio client can call the reservation REST API. Do not set **`HANOK_MCP_DISABLE`** unless you intentionally want to skip Hanok MCP wiring.
