...
| **agent-llm-service** | `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_DB=0`, `DB_URI`; LLM: `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_MODEL`; optional for provider checks: `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY`, `CARTESIA_API_KEY`, `CARTESIA_*`. **Hanok:** `HANOK_MCP_API_BASE_URL` = `https://hanok.convonetai.com` (or your Hanok service URL) when **hanok-table-service** is deployed; unset or mis-set URLs cause Hanok MCP to be skipped at runtime (see `convonet/routes.py` `_filter_mcp_servers_for_runtime`). |
...
| **hanok-table-service** | **`DB_URI`** (Postgres for reservations — can match agent-llm DB or a dedicated DB). **`HANOK_PUBLIC_BASE_URL`** = public origin for this service (e.g. `https://hanok.convonetai.com`) so Telnyx webhooks and self-referential URLs resolve. Optional: `ADMIN_DASHBOARD_TOKEN`, Telnyx keys (`TELNYX_API_KEY`, `TELNYX_CONNECTION_ID`, `TELNYX_FROM_NUMBER`), timezone and feature flags — see `kfood/hanok_table/config.py` and `kfood/README.md`. |

**Hanok Table (separate Cloud Run service):** Build with `docker/hanok-table.Dockerfile` (see `cloudbuild.yaml` or `cloudbuild-hanok-table.yaml`). The `kfood` path is a **git submodule** (see `.gitmodules` → `https://github.com/hjleepapa/8-telnyx.git`). Before Cloud Build, run `git submodule update --init kfood` so `kfood/hanok_table` is present (CI must clone with submodules, e.g. Cloud Build “Submodule connectivity” or a build step that runs this command). Map **`hanok.convonetai.com`** (or your hostname) in Cloudflare to the Cloud Run URL. On **agent-llm-service**, set **`HANOK_MCP_API_BASE_URL`** (or **`HANOK_PUBLIC_BASE_URL`**) to that same HTTPS origin (no trailing slash), e.g. `https://hanok.convonetai.com`, so Convonet’s MCP stdio client can call the reservation REST API. Do not set **`HANOK_MCP_DISABLE`** unless you intentionally want to skip Hanok MCP wiring.
