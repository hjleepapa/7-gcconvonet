...
|--------|------------------|
| **voice-gateway-service** | `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_DB=0`, `DB_URI`; `AGENT_LLM_URL` (agent-llm URL); Twilio: `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`, `TWILIO_TRANSFER_CALLER_ID` (or `TWILIO_CALLER_ID`), `FREEPBX_DOMAIN` or `FUSIONPBX_SIP_DOMAIN`; **`VOICE_GATEWAY_PUBLIC_URL`** or **`WEBHOOK_BASE_URL`** = `https://v2.convonetai.com` (so Twilio can reach `/twilio/voice_assistant/transfer_bridge` when we create outbound transfer calls). **STT/TTS (voice-gateway only):** `VOICE_STT_PROVIDER` and `VOICE_TTS_PROVIDER` must each be **one** id, e.g. `elevenlabs` or `deepgram` — **not** a pasted list like `deepgram|elevenlabs|cartesia` (that breaks parsing; the UI can override per session). Keys: `DEEPGRAM_API_KEY`, `DEEPGRAM_VOICE_ID`, `ELEVENLABS_*`, `CARTESIA_*`, `SPEECHMATICS_API_KEY` (STT and/or preview TTS), optional `SPEECHMATICS_TTS_VOICE`. See `docs/VOICE_GATEWAY_STT_TTS.md`. |
| **agent-llm-service** | **Memory:** use **≥ 2 GiB** on Cloud Run (LangGraph + multiple MCP clients often exceeds **512 MiB** → OOM → **503** and empty voice replies). `cloudbuild.yaml` sets `--memory=2Gi`. `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_DB=0`, `DB_URI`; LLM: `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_MODEL`; optional for provider checks: `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY`, `CARTESIA_API_KEY`, `CARTESIA_*`. **Hanok:** `HANOK_MCP_API_BASE_URL` = your deployed Hanok API origin (e.g. `https://hanok.convonetai.com`). The image must **`import hanok_table`** for MCP stdio (extend `docker/agent-llm.Dockerfile` with a wheel/copy if needed). Unset URL drops Hanok MCP at runtime (`convonet/routes.py` `_filter_mcp_servers_for_runtime`). |
...
| **hanok-table-service** (optional, **separate codebase / repo**) | **Not built from this repository.** Deploy Hanok reservation API from your Hanok project’s own repo and CI (e.g. its own Cloud Run service). Set **`DB_URI`**, **`HANOK_PUBLIC_BASE_URL`** (e.g. `https://hanok.convonetai.com`), and other vars per that project’s docs. Map **`hanok.convonetai.com`** in Cloudflare to that service’s URL. |

**Hanok Table (5th Cloud Run service):** Vendored package at repo **`./hanok_table/`** (populate from local `kfood/hanok_table` per `hanok_table/README.md`). Build/deploy with **`cloudbuild-hanok-table.yaml`** or the Hanok steps in **`cloudbuild.yaml`**. On **hanok-table-service**, set **`CONVONET_VOICE_ASSISTANT_URL`** (or **`VOICE_ASSISTANT_URL`**) to the full **call-center** voice UI URL, e.g. `https://v2.convonetai.com/voice_assistant` or your **call-center-service** `https://…run.app/voice_assistant`, so the Hanok landing page **Convonet voice (WebRTC)** button opens that page (Hanok and call-center are different origins). Point **agent-llm-service** at the running API with **`HANOK_MCP_API_BASE_URL`** = your Hanok URL (e.g. `https://hanok.convonetai.com`). The **agent-llm** image must **`import hanok_table`** (include the same `hanok_table/` tree in the agent-llm Docker build, or rely on repo `COPY . .` once `hanok_table/` is committed). Do not set **`HANOK_MCP_DISABLE`** unless you intentionally skip Hanok MCP.
