...
class CallCenterAgent {
    constructor() {
        ...
        // Track active call legs by SIP Call-ID to prevent duplicate processing
        // Map<callId, { sessionId, twilioCallSid, timestamp }>
        this.activeCallLegs = new Map();
    ...
    attachSessionEventHandlers(session, direction = 'inbound', dialedNumber = null) {
        ...
        session.on('ended', () => {
            ...
            // PATCH: Untrack call leg when session ends
            const identity = this.extractSessionIdentity(session);
            if (identity && identity.callId) {
                this.untrackCallLeg(identity.callId);
            }
        ...
        session.on('failed', (e) => {
            ...
            // PATCH: Untrack call leg when session fails
            const identity = this.extractSessionIdentity(session);
            if (identity && identity.callId) {
                this.untrackCallLeg(identity.callId);
            }
    ...
    handleIncomingCall(session) {
        ...
        // PATCH: Check for duplicate call leg by SIP Call-ID
        if (this.isDuplicateCallLeg(incomingIdentity)) {
            console.warn('🚫 Duplicate call leg detected (Twilio call forking). Rejecting with 486 Busy.', {
                callId: incomingIdentity.callId,
                twilioCallSid: incomingIdentity.twilioCallSid,
                sessionId: session.id,
                activeCallLegs: Array.from(this.activeCallLegs.entries())
            });
            this.rejectDuplicateCallLeg(session, incomingIdentity);
            return;
        }
        ...
        // PATCH: Track this call leg
        this.trackCallLeg(callId, session.id, callSid);
    ...
    isDuplicateCallLeg(identity) {
        if (!identity || !identity.callId) {
            return false;
        }

        // Check if we already have this Call-ID tracked
        return this.activeCallLegs.has(identity.callId);
    }

    trackCallLeg(callId, sessionId, twilioCallSid = null) {
        if (!callId) {
            console.warn('Cannot track call leg: missing callId');
            return;
        }

        this.activeCallLegs.set(callId, {
            sessionId: sessionId,
            twilioCallSid: twilioCallSid,
            timestamp: Date.now()
        });

        console.log('✅ Tracked call leg:', {
            callId,
            sessionId,
            twilioCallSid,
            totalActiveLegs: this.activeCallLegs.size
        });
    }

    untrackCallLeg(callId) {
        if (!callId) {
            return;
        }

        const removed = this.activeCallLegs.delete(callId);
        if (removed) {
            console.log('🗑️ Untracked call leg:', {
                callId,
                remainingLegs: this.activeCallLegs.size
            });
        }
    }

    rejectDuplicateCallLeg(session, identity) {
        try {
            // Reject with 486 Busy Here (standard SIP response for duplicate calls)
            session.terminate({
                status_code: 486,
                reason_phrase: 'Busy Here - Duplicate Call Leg'
            });

            console.log('✅ Rejected duplicate call leg with 486 Busy', {
                sessionId: session.id,
                callId: identity.callId,
                twilioCallSid: identity.twilioCallSid
            });

            // Clean up when this duplicate session ends
            session.on('ended', () => {
                console.log('Duplicate call leg session ended', session.id);
            });

            session.on('failed', () => {
                console.log('Duplicate call leg session failed', session.id);
            });
        } catch (error) {
            console.error('Failed to reject duplicate call leg:', error);
        }
    }
