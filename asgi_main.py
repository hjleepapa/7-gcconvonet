...
# 2. Initialize FastAPI (legacy hybrid entry; GCP uses convonet.voice_gateway_service instead)
api = FastAPI(
    title="Convonet Hybrid API",
    description="FastAPI gateway with integrated Flask monolith (legacy)",
    version="1.0.0"
)

# 3. Mount the Flask application at the root
# WSGIMiddleware wraps the WSGI (Flask) app so it can run inside the ASGI (FastAPI) loop.
