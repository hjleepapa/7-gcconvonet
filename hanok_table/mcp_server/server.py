...
from hanok_table.config import hanok_mcp_api_base_url, hanok_mcp_streamable_transport_security

_INSTRUCTIONS = (
    "Hanok Table reservation API tools. The POST /webhooks/hanok_table/variables endpoint may provide "
...
def _preorder_lines_from_simple(spec: str) -> list[dict[str, Any]]:
    ...
    from hanok_table.menu_catalog import resolve_menu_item_id
...
async def create_reservation(
    ...
    if preferred_locale is not None and str(preferred_locale).strip():
        from hanok_table.locale_prefs import normalize_preferred_locale
...
def reservation_voice_flow() -> str:
    """Suggested turn flow for voice booking (Convonet or compatible telephony)."""
    return (
