...
| Piece | Behavior |
|-------|-----------|
| **Process** | Single **`uvicorn hanok_table.app:app`** on Render (see root **`Procfile`**). |
...
| **`HANOK_MCP_TRANSPORT`** | For **local** `python -m hanok_table.mcp_server.server` only: `stdio` (default), `sse`, or `streamable-http`. Render mounted MCP ignores this; the FastAPI app chooses streamable HTTP. |
...
When **`POST /webhooks/hanok_table/variables`** is wired, map JSON fields to instruction placeholders. The MCP server’s built-in **`instructions`** and **`reservation_voice_flow`** prompt expect you to use them consistently:
