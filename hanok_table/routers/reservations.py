...
from hanok_table.datetime_wall import interpret_starts_at_as_utc_storage
from hanok_table.db import get_db
from hanok_table.menu_catalog import MENU_ITEMS
...
from hanok_table.preorder_calc import serialize_preorder
from hanok_table.reminders import schedule_demo_reminder_call
from hanok_table.seating_service import (
...
from hanok_table.table_allocation import floor_slot_start
from hanok_table.schemas_res import (
    ReservationCreate,
    ReservationRead,
...
def _hanok_table_null_placeholder_bundle(body: ReservationUpdate) -> bool:
...
def _preorder_null_clears_cart(body: ReservationUpdate) -> bool:
    ...
    if "preorder" not in body.model_fields_set or body.preorder is not None:
        return False
    if _hanok_table_null_placeholder_bundle(body):
        return False
...
def _apply_reservation_update(db: Session, row: Reservation, body: ReservationUpdate) -> bool:
    ...
    if (
        ...
        from hanok_table.seating_service import release_and_promote_after_cancel
...
def _patch_at_status_in_thread(row_id: int, flat: dict[str, Any]) -> tuple[ReservationRead, bool]:
    ...
    from hanok_table import db as db_mod
    ...
    if "{{" in v or "}}" in v:
        raise HTTPException(
            ...
            detail=(
                ...
                "(name + phone is the primary lookup; map phone to hanok_table_end_user_target from dynamic variables)."
            ),
        )
    return v
...
def _flat_guest_identity_for_amend(flat: dict[str, Any]) -> tuple[str, str] | None:
    ...
    if gp is None:
        gp = (
            ...
            or flat.get("hanok_table_end_user_target")
    ...
    if "{{" in phone or "}}" in phone:
        raise HTTPException(
            ...
            detail=(
                ...
                "(e.g. hanok_table_end_user_target), not literal {{…}} in the URL."
            ),
        )
...
def lookup_reservation_by_guest_phone(
    ...
    Map Telnyx `hanok_table_end_user_target` into query param `phone` or `guest_phone` from the tool.
    """
    raw_phone = ((phone or guest_phone) or "").strip()
    if len(raw_phone) < 3:
        raise HTTPException(
    ...
    if "{{" in phone or "}}" in phone:
        raise HTTPException(
            ...
            detail=(
                ...
                "caller number (e.g. hanok_table_end_user_target), not literal {{…}} in the URL."
            ),
        )
    if guest_name and ("{{" in guest_name or "}}" in guest_name):
        raise HTTPException(
