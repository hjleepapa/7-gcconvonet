...
Caller resolution gathers `hanok_table_end_user_target`, `caller_number`, and `from` (flat and
...
1. Bind the assistant’s dynamic-variables request to ``POST …/webhooks/hanok_table/variables`` and map every key above
   (plus caller lookup fields) into template variables. Use ``hanok_table_end_user_target`` for tools so PSTN maps to DB rows.
...
3. **Validate wiring:** Place a test booking with preorder food total ≥ $300 (after discount: ``food_total_cents`` in
   ...
   not match the DB row—fix ``hanok_table_end_user_target`` / phone mapping first.
...
**Caller ID (shared demo line):** responses may include ``caller_phone_hanok_table``, ``caller_phone_normalized``,
...
from hanok_table.db import get_engine
from hanok_table.models import Reservation, ReservationStatus
from hanok_table.phone_normalize import phone_lookup_variants, to_e164_us
from hanok_table.preorder_calc import preorder_summary_text
from hanok_table.reminders import build_reminder_speak_text, hanok_table_hangup, hanok_table_speak
from hanok_table.seating_service import waitlist_queue_metadata
from hanok_table.locale_prefs import assistant_locale_hint
from hanok_table.webhook_payload import extract_caller_number
from hanok_table.routers.reservations import (
...
def _hanok_table_template_alias_variants(profile: dict[str, Any]) -> None:
...
def _profile_from_db(caller: str | None) -> dict[str, Any] | None:
    ...
    if not variants:
        return None
    get_engine()
    from hanok_table.db import SessionLocal
            ...
            out = {
                ...
                **(_locale_profile_fields(row)),
                ...
                "demo_reminder_note": (
                    ...
                    "webhook to POST …/webhooks/hanok_table/call-control so the call plays TTS when answered."
                ),
            }
            if len(pool) > 1:
...
async def hanok_table_call_control(request: Request) -> dict[str, str]:
    ...
    if event_type == "call.answered":
        ...
        ok, tag = hanok_table_speak(call_control_id, msg)
        if ok:
            with _hanok_cc_lock:
    ...
    if event_type in ("call.speak.ended", "call.speak.completed", "call.speak.stopped"):
        ...
        with _hanok_cc_lock:
            if call_control_id in _hanok_cc_ids:
                ...
                do_hangup = True
        if do_hangup:
            hanok_table_hangup(call_control_id)
...
def _enrich_caller_identification_for_profile(profile: dict[str, Any], caller: str | None) -> None:
    ...
    raw = (caller or "").strip()
    profile["caller_phone_hanok_table"] = raw
    ...
    profile["guest_lookup_identification_hint"] = (
        "Caller phone was not in the webhook payload (bind Telnyx `hanok_table_end_user_target` / "
    ...
    if multi:
        ...
        profile["guest_lookup_name_for_tools"] = ""
        profile["guest_lookup_identification_hint"] = (
            "The caller's phone is already known from the carrier (caller_phone_normalized / caller_phone_hanok_table). "
...
async def dynamic_webhook_variables(
    ...
    if "demo_reminder_note" not in profile:
        profile["demo_reminder_note"] = (
            ...
            "Use the Call Control webhook POST …/webhooks/hanok_table/call-control so answered calls speak the reminder."
        )
    _ensure_premium_concierge_on_profile(profile)
    _enrich_caller_identification_for_profile(profile, caller)
    _hanok_table_template_alias_variants(profile)
    return profile
