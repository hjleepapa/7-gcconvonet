...
from hanok_table.config import (
    ...
    hanok_table_voice_from_number,
)
from hanok_table.models import Reservation
from hanok_table.phone_normalize import to_e164_us
...
def _demo_reminder_worker(
    ...
    if database_url():
        from hanok_table.db import SessionLocal, get_engine
        from hanok_table.models import Reservation
    ...
    status = _place_hanok_table_reminder_call(to_e164=to_e164, state_obj=state_obj)
...
def _place_hanok_table_reminder_call(
    *,
    to_e164: str,
    state_obj: dict[str, Any],
) -> str:
    key = hanok_table_voice_api_key()
    conn = hanok_table_voice_connection_id()
    from_raw = hanok_table_voice_from_number()
    if not key or not conn or not from_raw:
        ...
        return "demo_skipped_no_voice_provider_config"
    ...
    if base:
        ...
        body["webhook_url"] = f"{base}/webhooks/hanok_table/call-control"
    ...
    try:
        ...
        return "hanok_table_call_initiated"
    except urllib.error.HTTPError as e:
        detail = e.read().decode("utf-8", errors="replace")[:800]
        logger.warning("Hanok reminder: Telnyx HTTP %s: %s", e.code, detail)
        return f"hanok_table_error_http_{e.code}"
    except Exception:
        logger.exception("Hanok reminder: Telnyx call failed")
        return "hanok_table_error_exception"


def hanok_table_speak(call_control_id: str, text: str) -> tuple[bool, str]:
...
def _hanok_table_hangup_call_already_ended(detail: str) -> bool:
...
def hanok_table_hangup(call_control_id: str) -> bool:
    key = hanok_table_voice_api_key()
    if not key or not call_control_id:
        return False
    try:
        ...
        detail = e.read().decode("utf-8", errors="replace")[:800]
        if e.code == 422 and _hanok_table_hangup_call_already_ended(detail):
            logger.info(
