...
from hanok_table.routers.reservations import _reservation_read_response
...
def test_book_rejects_when_full(seating_env: None, db_session: Session) -> None:
    from hanok_table.seating_service import SeatingUnavailableError
...
def test_same_slot_reseat_invokes_promote_for_waitlist(
    ...
    monkeypatch.setattr("hanok_table.seating_service.promote_waitlist", _capture)
...
def test_waitlist_promotion_schedules_reminder_for_voice(
    ...
    monkeypatch.setattr(
        "hanok_table.seating_service.schedule_reminder_on_table_allocated",
        _capture,
    )
